...
public class ImageNeuralNetwork {
	...
	public void processWhatIs() throws IOException {
		...
		int tmp = 0;
		final int pom = img.getWidth()/6, pomheight = img.getHeight();
		List<String> tmpList = new ArrayList<>();

		if (img.getWidth() > 200) {
			for (int i = 1; i <= 6; i++) {
				final BufferedImage subimage = img.getSubimage(tmp, 0, pom, pomheight);
				ImageMLData input = new ImageMLData(subimage);
				int winner = 0;
				tmp += pom;
				input.downsample(this.downsample, true, this.downsampleHeight, this.downsampleWidth, 1, -1);

				ImageIO.write(toBufferedImage(input.getImage()), "png", new File("./output/out"+i+".png"));
//                for (int x = 0; x < 5; x++){
//                    winner = this.network.winner(input);
//                    tmpList.add(this.neuron2identity.get(winner));
//                }
				winner = this.network.winner(input);
...
//        System.out.println(tmpList);
		System.out.println("What is: " + filename + ", it seems to be: " + output);
